...
Sphere::Sphere()
{
	...
	double deltaTheta = 180 / (rings + 2);
	double deltaPhi = 360 / rings;
	...
	for (int i = 0; i < 2; i++){
		face_vhandles.clear();
		face_vhandles.push_back(vertices[(rings - 1) * rings + i]);
		face_vhandles.push_back(northCap);
		face_vhandles.push_back(vertices[(rings - 1) * rings + (i + 1) % rings]);
		mesh.add_face(face_vhandles);
	}
